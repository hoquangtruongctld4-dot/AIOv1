...
namespace subphimv1.Services
{
    ...
    public static class ApiService
    {
        ...
        {
            if (string.IsNullOrEmpty(_jwtToken)) return new GetResultsResponse { NewLines = null, IsCompleted = true, ErrorMessage = "Yêu cầu đăng nhập." };
            Func<Task<GetResultsResponse>> apiCall = async () =>
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    var result = JsonSerializer.Deserialize<GetResultsResponse>(responseBody, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                    return result;
                }
                return new GetResultsResponse { NewLines = null, IsCompleted = true, ErrorMessage = $"Lỗi server: {response.ReasonPhrase}" };
            };

            Func<GetResultsResponse> errorFactory = () => new GetResultsResponse { NewLines = null, IsCompleted = true, ErrorMessage = "Lỗi kết nối khi lấy kết quả." };
        ...
        public static async Task<(bool success, string message, UserDto user)> LoginAsync(string username, string password, string hwid, bool isRetry = false)
        {
            ...
            catch (Exception ex) when (ex is HttpRequestException || ex is TaskCanceledException)
            {
                if (!isRetry)
                {
                    ...
                    if (fallbackSuccess)
                    {
                        return await LoginAsync(username, password, hwid, true);
                    }
                    else {}
                }
                else { }
                return (false, "Không thể kết nối tới máy chủ. Vui lòng kiểm tra kết nối mạng.", null);
            }
            catch (Exception ex)
            {
        ...
        public static async Task<(bool success, byte[] audioData, string errorMessage)> GenerateAioTtsAsync(string language, string voiceId, double rate, string text)
        {
            if (string.IsNullOrEmpty(_jwtToken))
            {
            ...
            {
                var requestPayload = new { Language = language, VoiceId = voiceId, Rate = rate, Text = text };
                var content = new StringContent(JsonSerializer.Serialize(requestPayload), Encoding.UTF8, "application/json");
        ...
        public static async Task<(bool success, string jobId, string errorMessage)> StartAioTtsBatchJobAsync(string srtContent, string language, string voiceId, double rate, string audioFormat = "mp3")
        {
